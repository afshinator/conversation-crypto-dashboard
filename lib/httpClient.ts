/**
 * HTTP client for outbound requests (e.g. from backend API routes).
 *
 * Use this in server-side code only (e.g. api/fetch.ts calling CoinGecko).
 * Do not use in the React app for calling your own API—use plain fetch('/api/...') there.
 *
 * ## Usage
 *
 * ```ts
 ...
 *
 * const res = await HttpClient.request<GlobalResponse>(
 *   'https://api.coingecko.com/api/v3/global',
 *   { params: { x_cg_demo_api_key: process.env.COINGECKO_DEMO_API_KEY }, timeout: 30_000 }
 * );
 *
 * if (!res.isOk) {
 *   if (res.status === 0) return { error: 'Network or timeout' };
 *   if (res.status === HTTP_STATUS['RATE_LIMITED-429']) return { error: 'Rate limited' };
 ...
 * ## Behavior
 *
 * - Never throws on HTTP errors: returns { data, status, headers, isOk, error }.
 * - Status 0: network failure, CORS, or timeout (so you can show "Offline" or "Request timed out").
 * - Raw headers are returned for ETag / If-None-Match and 304 handling if needed.
 * - 429 is returned as status without throwing so callers can implement Retry-After.
 * - Optional timeout (ms) with AbortController; defaults to DEFAULT_TIMEOUT_MS.
 */
